...
- Writing or reviewing React/TSX tests (`.test.tsx`, `.spec.tsx`, `__tests__/`)
- Choosing a component test runner (RTL + Vitest/Jest, or Playwright/Cypress CT)
- Selecting queries (role/label/text vs `data-testid`), simulating user interaction, asserting async behavior
- Mocking network calls with MSW, wrapping providers in test helpers, testing custom hooks
- Adding accessibility assertions or deciding when to reach for Playwright/Cypress over RTL
