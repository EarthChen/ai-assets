...
A role is a mission plus a scope. The default worker is a **builder scoped to a domain** — `pay-wk-frontend`, `pay-wk-backend` — because parallel code-writing is the normal shape of a swarm. Reviewer, researcher, tester, and planner are auxiliary roles, spawned when a phase needs them (usually one at a time).
...
- `<name>` — worker agent name: plan slug prefix + role/domain, e.g. `pay-wk-frontend` (the prefix keeps names unique across concurrent plans)
...
- `<swarm>` — the plan directory (`.herdr-swarm/<plan-id>`); substitute the concrete path — workers see only their own plan's paths
...
- You report to <orchestrator>. Short questions, answers, and status travel as
  ...
  file drop in <swarm>/inbox/<name>/; never inject text into a blocked agent.
...
- Anything longer than ~40 lines, and every artifact, moves as a file under
  <swarm>/ — messages carry the path, not the content.
- Clear <swarm>/inbox/<name>/ at every task boundary: before reading a new
  brief and before writing a result.

Tasks
- An assignment is a pointer to a brief under <swarm>/tasks/. Read the brief
...
- Keep <swarm>/tasks/<task-id>/progress.md current: after each significant
...
- If you ever doubt your prior progress — typically right after your context
  ...
  <swarm>/state/<name>.md before acting. Never continue on summary memory.
- A task that outgrows you: write sub-briefs under <swarm>/tasks/<task-id>/
...
Results
- Write <swarm>/tasks/<task-id>/result.md with exactly these sections:
...
- Then reply in your pane with ONE line: "<task-id> done: <swarm>/tasks/<task-id>/result.md"
...
- **Contracts at the seams.** Where domains touch, the orchestrator writes `<swarm>/contracts/<name>.md` (API shapes, event schemas, shared types) before spawning; both briefs reference it. Sharing a contract makes the two builders automatic peers. A builder changes its side of a contract only through the orchestrator.
