...
| `working` | Never prompt. Drop a file into `.herdr-swarm/inbox/<name>/`; the recipient clears its inbox at task boundaries. |
...
- Path: `.herdr-swarm/inbox/<recipient>/<sender>-<topic>.md`.
...
- A worker's completion is ONE line: `"<task-id> done: .herdr-swarm/tasks/<task-id>/result.md"` or `"<task-id> blocked: <reason>"`. Workers append `ctx=<totalTokens>` only when the orchestrator has instructed self-reporting (telemetry escape hatch).
...
- **Eligibility**: only across a shared contract boundary. Two briefs referencing the same `.herdr-swarm/contracts/<name>.md` make those workers automatic peers — the orchestrator names them in each other's preamble. Ad-hoc peer links: ask the orchestrator, who announces the link to both sides.
